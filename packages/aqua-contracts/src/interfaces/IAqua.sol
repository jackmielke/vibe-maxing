...
interface IAqua {
    ...
    /// @dev Only callable by trusted delegates
    /// @param maker The maker address (may be on a different chain)
    /// @param app The implementation contract
    /// @param strategy Initialization data passed to the strategy
    /// @param tokens Array of token addresses to approve
    /// @param amounts Array of balance amounts for each token
    ...
    /// @dev Only callable by trusted delegates
    function dockOnBehalfOf(address maker, address app, bytes32 strategyHash, address[] calldata tokens) external;
    ...
    /// @dev Only callable by trusted delegates
    function pullOnBehalfOf(address maker, address app, bytes32 strategyHash, address token, uint256 amount, address to) external;
    ...
    /// @dev Only callable by trusted delegates
    function pushOnBehalfOf(address maker, address app, bytes32 strategyHash, address token, uint256 amount, address payer) external;
}
